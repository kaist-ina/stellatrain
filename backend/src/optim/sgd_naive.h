#ifndef OPTIM_SGD_NAIVE_H
#define OPTIM_SGD_NAIVE_H
...
class SGDNaive : public SparseOptimizer {

private:
    float m_momentum, m_weight_decay, m_dampening;
    bool m_nestrov, m_maximize, m_smart_momentum;
    std::unordered_map<std::string, at::Tensor> m_optim_state_b;
    std::unordered_map<std::string, at::Tensor> m_optim_state_last;
    std::mutex m_optim_state_b_mutex_;
    uint32_t m_iter;

public:
    SGDNaive() : m_momentum(0), m_weight_decay(0), m_dampening(0), m_nestrov(false), m_maximize(false), m_iter(0) {
